...
namespace Altinn.Platform.Authorization.IntegrationTests.MockServices
{
    public class HapRolesMock : IRoles
    {
        ...
        private static string GetRolesPath(int coveredByUserId)
        {
            string unitTestFolder = Path.GetDirectoryName(new Uri(typeof(RolesMock).Assembly.CodeBase).LocalPath);
            return Path.Combine(unitTestFolder, $"../../../Data/Roles/user_{coveredByUserId}/hap/roles.json");
        }
    }
}
